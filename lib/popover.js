...
function Popover( translationPair, _locale, glotPress ) {
	...
	var getPopoverTitle = function() {
		return 'Translate <a title="Help & Instructions" target="_blank" href="https://en.support.wordpress.com/in-page-translator/"><span class="noticon noticon-help"></span></a><a title="View in GlotPress" href="' + glotPress.getPermalink( translationPair ) + '" target="_blank" class="gpPermalink"><span class="noticon noticon-external"></span></a>';
	};
...
function getInputForm( translationPair ) {
	...
	for ( var i = 0; i < item.length; i++ ) {
		...
		pair.find( 'textarea' ).text( item[ i ].getText() ).attr( 'placeholder', 'Translate to ' + locale.getLanguageName() );
...
function getHtmlTemplate( popoverType ) {
	...
	case 'new-translation':
		return jQuery(
			'<form class="community-translator">' +
			'<div class="original"></div>' +
			'<p class="context"></p>' +
			'<p class="comment"></p>' +
			'<p class="info"></p>' +
